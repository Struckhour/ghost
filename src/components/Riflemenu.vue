<script setup lang="ts">
   ...
   let shower = computed(() => {
    ...
    let scopeMenu = computed(() => {
    return ref(props.scopeMenu);
  })

    const isOpen2 = () => {
      shower.value.value = !shower.value.value;
      emit('rifleMenuClicked', shower.value.value)
      scopeMenu.value.value = false;
      emit('scopeMenu', scopeMenu.value.value);
    }
...
<template>
  ...
  <div class="grid md:grid-cols-2 grid-cols-1 md:w-[30rem] w-[15rem] bg-slate-900 bg-opacity-10 -translate-x-[4rem] z-40 absolute">
      ...
      >
        <div @click="changeRifle(rifle)" class="px-4 md:py-0 py-2 md:my-0 my-1  z-40 md:text-[.85rem] text-base text-cyan-300 hover:bg-cyan-700 hover:text-indigo-100">
          {{rifle}}
        </div>
